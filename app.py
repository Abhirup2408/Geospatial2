...
def correct_names(text):
    ...
    my_list = []
    coordinates = []  # List to store coordinates for map visualization
    for item in first_index:
        ...
        # Append coordinates as dictionary for map plotting
        coordinates.append({"latitude": latitude, "longitude": longitude})
    
    return my_list, coordinates
...
if st.button("Find Locations"):
    ...
    if user_input:
        corrected_locations, coordinates = correct_names(user_input)
